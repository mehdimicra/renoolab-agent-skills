...
const PUBLIC_BASE_URL = "https://renoolab.fr/.well-known/agent-skills/packages/v0.5.1";
...
try {
  ...
  for (const entry of index.skills) {
    ...
    const firstArchive = await readFile(join(firstOutput, "packages", "v0.5.1", archiveName));
    const secondArchive = await readFile(join(secondOutput, "packages", "v0.5.1", archiveName));
